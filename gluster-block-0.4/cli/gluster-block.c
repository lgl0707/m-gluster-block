...
# define  GB_MODIFY_HELP_STR  "gluster-block modify <volname/blockname> "      \
                                "[auth <enable <username password>|disable>] [size <size>] "       \
                                "[force] [--json*]"
...
glusterBlockCliRPC_1(void *cobj, clioperations opt)
{
  ...
  blockModifyCliUserInfo *modify_obj;
  ...
  case MODIFY_CLI:
    ...
    if (block_modify_cli_1(modify_obj, &reply, clnt) != RPC_SUCCESS) {
      LOG("cli", GB_LOG_ERROR, "%s block modify auth on volume %s failed",
          clnt_sperror(clnt, "block_modify_cli_1"), modify_obj->mobj.volume);
      goto out;
    }
    break;
...
static int regexdec(char *reg_str, char *str)
{
  char ebuff[256];
  int ret;
  int cflags;
...
glusterBlockModify(int argcount, char **options, int json)
{
  ...
  blockModifyCliUserInfo blkinfo = {};
  ...
  if (argcount < 4 || argcount > 7) {
    MSG(stderr, "Inadequate arguments for modify2:\n%s", GB_MODIFY_HELP_STR);
    return -1;
  }
  ...
  if (!strcmp(options[optind], "auth")) {
    ...
    if(ret > 0) {
      mobj.auth_mode = ret;
      if((options[optind] == NULL) || (options[optind+1] == NULL))
      {
	MSG(stderr, "Inadequate arguments for modify1:\n%s", GB_MODIFY_HELP_STR);
	return -1;
      }
      LOG("cli", GB_LOG_DEBUG, "args: %d %s", optind ,options[optind]);
      LOG("cli", GB_LOG_DEBUG, "args++: %d %s", optind ,options[optind+1]);

      char *reg_username = "^[0-9A-Za-z]{1,20}$";
      char *reg_passwd = "^[0-9A-Za-z]{12,16}$";
      int result_username = regexdec(reg_username, options[optind]);
      int result_passwd = regexdec(reg_passwd, options[optind+1]);
      if(!(result_username && result_passwd))
      {
	MSG(stderr, "username or password input invalid! The Password is 12-16 digits,Username does not exceed 20 digits,and Contains only letters and numbers ");
        return -1;
      }

      strcpy(blkinfo.blk_username, options[optind++]);
      strcpy(blkinfo.blk_passwd, options[optind++]);
    } 
    else if(ret == 0)
    {
      mobj.auth_mode = ret;
    ...
    ret = glusterBlockCliRPC_1(&blkinfo, MODIFY_CLI);
    if (ret) {
      LOG("cli", GB_LOG_ERROR,
  ...
  while (argcount - optind > 2) {
    ...
    case GB_CLI_CREATE_AUTH:
      ...
      if(ret > 0) {
        ...
        strcpy(cobj.username, options[optind++]);
        strcpy(cobj.passwd, options[optind++]);
      }
      else if(ret == 0){
        cobj.auth_mode = ret;
      }
