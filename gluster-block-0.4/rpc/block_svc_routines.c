...
convertTypeCreate2ToCreate(blockCreate2 *blk_v2, blockCreate *blk_v1)
{
  ...
  GB_STRCPYSTATIC(blk_v1->username, blk_v2->username);
  ...
  if (blk->json_resp) {
    ...
    if (!errCode && mobj->auth_mode) {
      GB_ASPRINTF(&tmp3, "IQN: %s%s\nUSERNAME: %s\nPASSWORD: %s\n%s%s",
                  ...
                  mobj->username, mobj->passwd, tmp?tmp:"", tmp2?tmp2:"");
    } else {
      GB_ASPRINTF(&tmp3, "IQN: %s%s\n%s%s",
                  GB_TGCLI_IQN_PREFIX, info->gbid,
                  tmp?tmp:"", tmp2?tmp2:"");
...
blockResponse *
block_modify_cli_1_svc_st(blockModifyCliUserInfo *userblk, struct svc_req *rqstp)
{
  int ret = -1;
  blockModify mobj = {{0}};
  ...
  char *errMsg = NULL;
  blockServerDefPtr list = NULL;
  blockModifyCli *blk = &(userblk->mobj);
...
//    if(info->passwd[0] == '\0') {
      ...
      GB_STRCPYSTATIC(mobj.passwd, userblk->blk_passwd);
      GB_METAUPDATE_OR_GOTO(lock, glfs, blk->block_name, blk->volume,
                            ret, errMsg, out, "USERNAME: %s\n", userblk->blk_username);
      GB_STRCPYSTATIC(mobj.username, userblk->blk_username);
//    } else {
                             ...
                             blockResponse *reply)
{
  ...
  if (blk->json_resp) {
    ...
    if (blk->auth_mode && savereply->iqn) {
      json_object_object_add(json_obj, "USERNAME",
                             GB_JSON_OBJ_TO_STR(cobj->username));
      json_object_object_add(json_obj, "PASSWORD",
...
block_create_cli_1_svc_st(blockCreateCli *blk, struct svc_req *rqstp)
{
  ...
  if (blk->auth_mode) {
    ...
    GB_STRCPYSTATIC(cobj.passwd, blk->passwd);
    GB_METAUPDATE_OR_GOTO(lock, glfs, blk->block_name, blk->volume,
                            errCode, errMsg, exist, "USERNAME: %s\n", blk->username);
    GB_STRCPYSTATIC(cobj.username, blk->username);
  }
...
blockValidateCommandOutput(const char *out, int opt, void *data)
{
  ...
  case MODIFY_SRV:
    if (mblk->auth_mode) {
      ...
      /* userid set validation */
      GB_OUT_VALIDATE_OR_GOTO(out, out, "username set failed for: %s", mblk,
                      mblk->volume,
                      "Parameter username is now '%s'.", mblk->username);
                           ...
                           blockResponse *reply)
{
  ...
  if (blk->json_resp) {
    ...
    if (GB_ASPRINTF(&tmp, "NAME: %s\nVOLUME: %s\nGBID: %s\nSIZE: %s\n"
                    ...
                    info->mpath, info->username, info->passwd) == -1) {
      goto out;
    }
    for (i = 0; i < info->nhosts; i++) {
      if (blockhostIsValid (info->list[i]->status)) {
...
bool_t
block_modify_cli_1_svc(blockModifyCliUserInfo *blk, blockResponse *reply,
                       struct svc_req *rqstp)
{
  int ret;
